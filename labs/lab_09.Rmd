...
**A1.** I thought a grouped bars chart would be the best way to show off the difference of votes between Dan Cox and Larry Hogan in each county. Separating the data by county allowed me to put the percentage of votes Cox received and percentage of votes Hogan received side-by-side.

https://datawrapper.dwcdn.net/ynLjX/1/

```{r}
md_gov_county <- read_csv ("data/md_gov_county.csv")
md_gov_county_18 <- read_csv ("data/md_gov_county_18.csv")
md_gov <- md_gov_county |> left_join(md_gov_county_18, by = "County")
md_gov <- md_gov |> 
  mutate(
    total_votes = Cox + Moore + Lashar + Wallace + Harding + Sefcik + `Write-ins.x`+ Hogan + Jealous + Quinn + Schlakman + `Write-ins.y`,
...
**A2.** I think a story would be why election day voting became more popular in the last election. Talking to a bunch of people to see how they voted and why they did it that way would be interesting.

https://datawrapper.dwcdn.net/w2VP3/1/
...
**A3.** The story here is definitely digging into Montgomery County and trying to figure out why they have so many overdose calls. I know Montgomery County is one of the most populated in the state, but the really high overdose counts are concerning and should be looked into. I would be curious to see if it is the same house that calls often in each zipcode because that would give me the impression that these are at parties.

https://datawrapper.dwcdn.net/Mfuvv/1/

```{r}
montgomery_2022_overdoses <- read_csv("data/montgomery_2022_overdoses.csv")
total <- montgomery_2022_overdoses |> 
  group_by(zip) |> 
  summarise (
    total = n()
  ) |> 
  mutate(
    ...
    pct_calls = total / total_calls * 100
  ) 
```
